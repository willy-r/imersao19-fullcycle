...
class VideoAdmin(admin.ModelAdmin):
    ...
    def get_readonly_fields(self, request: HttpRequest, obj: Any | None) -> list[str]:
        if not obj:
            return ['video_status','is_published', 'published_at', 'num_likes', 'num_views', 'author']
        
        return ['video_status','published_at', 'num_likes', 'num_views', 'author']
    ...
    def finish_upload_video_view(self, request, id):
        ...
        try:
            ...
            return JsonResponse({'error': 'Vídeo não encontrado.'}, status=404)
        except (VideoMediaNotExistsException, VideoMediaInvalidStatusException, VideoChunkUploadException) as exc:
